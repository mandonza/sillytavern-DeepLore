...
| **AI Error Fallback** | `keyword` | Dropdown | What to inject when AI search errors out. `keyword`: fall back to keyword-matched results. `keep_previous`: keep injecting the last successful AI selection (plus constants/bootstrap) until a new search succeeds; degrades to keyword matching if no previous selection exists. `constants_only`: constants only. `bootstrap_only`: constants and bootstrap entries. `none`: nothing. |
